...
html,body{width:100%;height:100%;overflow:hidden;font-family:'Pretendard','Noto Sans KR',-apple-system,sans-serif;background:#111;color:#fff}

.presenter{display:flex;flex-direction:column;height:100vh}

/* ─── Top Bar ─── */
.top-bar{display:flex;align-items:center;justify-content:space-between;padding:12px 24px;background:#1a1a1a;border-bottom:1px solid #333;flex-shrink:0;gap:16px;z-index:10}
.top-bar .title{font-size:14px;font-weight:600;color:#ccc;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0}
.top-bar .controls{display:flex;align-items:center;gap:8px;flex-shrink:0}
.btn{padding:6px 14px;border:1px solid #444;border-radius:6px;background:#222;color:#ddd;font-size:13px;cursor:pointer;transition:all .15s;font-family:inherit;white-space:nowrap}
.btn:hover{background:#333;border-color:#666}
.btn.active{background:#6C5CE7;border-color:#6C5CE7;color:#fff}
.counter{font-size:13px;color:#888;font-variant-numeric:tabular-nums;min-width:60px;text-align:center}

/* ─── Stage ─── */
.stage{flex:1;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden;background:#111}
.slide-frame{width:1080px;height:1440px;transform-origin:center center;position:relative;border-radius:4px;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,.5)}
...
/* ─── Nav Arrows ─── */
.nav-arrow{position:absolute;top:50%;transform:translateY(-50%);width:48px;height:80px;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.4);border:none;color:#fff;font-size:24px;cursor:pointer;border-radius:8px;transition:background .15s;z-index:5;backdrop-filter:blur(4px)}
...
/* ─── Thumbnail Strip ─── */
.thumb-strip{display:flex;align-items:center;gap:8px;padding:12px 24px;background:#1a1a1a;border-top:1px solid #333;overflow-x:auto;flex-shrink:0;z-index:10}
.thumb-strip::-webkit-scrollbar{height:4px}
.thumb-strip::-webkit-scrollbar-thumb{background:#444;border-radius:2px}
.thumb{width:54px;height:72px;border-radius:4px;border:2px solid transparent;cursor:pointer;overflow:hidden;flex-shrink:0;transition:border-color .15s,transform .15s;background:#222}
.thumb:hover{border-color:#888;transform:scale(1.05)}
.thumb.active{border-color:#6C5CE7;transform:scale(1.1)}
.thumb img{width:100%;height:100%;object-fit:cover}
.thumb .thumb-num{width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;color:#888}
</style>
</head>
<body>
<div class="presenter">
  <div class="top-bar">
    ...
    <div class="controls">
      <button class="btn" id="btn-fullscreen" onclick="toggleFullscreen()" title="Fullscreen (F)">⛶ Fullscreen</button>
